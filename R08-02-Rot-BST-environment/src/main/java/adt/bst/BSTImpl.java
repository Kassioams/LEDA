...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	...
	public BSTNode<T> search(T element) {
		BSTNode<T> retorn = (BSTNode<T>) new BSTNode();

		if (element != null) {
			BSTNode<T> aux = this.root;
			boolean find = false;

			while (!aux.isEmpty() && !find) {
	...
	private void removeDegree2(BSTNode<T> node) {
		BSTNode<T> sucessor = (BSTNode<T>) sucessor(node.getData());
		int grauSucessor = degree(sucessor);
		node.setData(sucessor.getData());

		if (grauSucessor == 0) {
			sucessor.setData(null);
		} else if (grauSucessor == 1) {
			removeDegree1(sucessor);
		} else if (grauSucessor == 2) {
			removeDegree2(sucessor);
		}
	}

	private void removeDegree1(BSTNode<T> node) {
